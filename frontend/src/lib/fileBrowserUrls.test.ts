...
describe("fileBrowserUrls", () => {
  it("normalizes empty and trailing-slash bases", () => {
    ...
    expect(normalizeBaseUrl("https://example.com/root/")).toBe("https://example.com/root");
  });
  ...
  it("builds browser and mutation URLs relative to the base", () => {
    ...
    expect(buildUploadUrl("", "nested/folder")).toBe("/files/upload?path=nested%2Ffolder");
    expect(buildCreateFolderUrl("", "")).toBe("/folder/create");
  });

  it("builds download URLs and preserves absolute inputs", () => {
    expect(buildFileDownloadUrl("", 7, "poster.png")).toBe("/upload/7/poster.png");
    expect(withBaseUrl("https://example.com/root", "https://cdn.example.com/file")).toBe(
